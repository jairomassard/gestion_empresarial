<template>
  <div class="upload-venta-mensual">
    <h2>Cargar Ventas Mensuales</h2>
    <div class="instructions">
      <p>Suba un archivo Excel (.xlsx) con las ventas mensuales. Descargue la plantilla para usar el formato correcto.</p>
      <p><strong>Columnas requeridas (en la primera fila):</strong> Año, PDV, Mes, Venta</p>
      <p><strong>Notas:</strong> Use nombres de meses en español (Enero, Febrero, etc.). La columna Venta puede incluir el símbolo $ y separadores de miles (.).</p>
    </div>
...
export default {
  ...
  methods: {
    ...
    downloadTemplate() {
      ...
      const headers = ['Año', 'PDV', 'Mes', 'Venta'];

      // Datos de ejemplo
      const exampleData = [
        '2025', 'AAA - Caja Grand Boulevard', 'Enero', '$39.541.846'
      ];
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Plantilla Ventas Mensuales');

      // Descargar el archivo
      XLSX.writeFile(wb, 'plantilla_ventas_mensuales.xlsx');
    }
  },
  mounted() {
    if (!this.token) {
