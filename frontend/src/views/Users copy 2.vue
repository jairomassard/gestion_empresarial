<template>
  <div class="users">
    ...
    <form @submit.prevent="isEditing ? updateUser() : createUser()">
      ...
      <div class="form-group">
        <label for="nombre">Nombres:</label> <!-- Cambié la etiqueta pero uso "nombre" en v-model -->
    ...
    <table>
      ...
      <tbody>
        <tr v-for="usuario in usuarios" :key="usuario.idusuario">
          ...
          <td>{{ getPerfilNombre(usuario.perfil_id) }}</td> <!-- Cambié de perfilid a perfil_id -->
          <td>{{ usuario.nombre || 'N/A' }}</td> <!-- Cambié a nombre -->
...
export default {
  ...
  setup() {
    ...
    const usuario = ref('');
    const password = ref('');
    const estado = ref(true); // Añadimos estado, por defecto true
    const errorMessage = ref('');
    const isEditing = ref(false);
    const editingUserId = ref(null);

    // Relajamos el filtro para el superadministrador: mostramos todos los perfiles
    const perfilesFiltrados = computed(() => {
      return perfiles.value; // Superadmin ve todos los perfiles, sin filtro por idcliente
      // Si necesitas volver al filtro: 
    ...
    const editUser = (user) => {
      ...
      perfil_id.value = user.perfil_id || user.perfilid || ''; // Soporte para ambos casos
      nombre.value = user.nombre || '';
      apellidos.value = user.apellidos || '';
      usuario.value = user.usuario || '';
      password.value = '';
      estado.value = user.estado; // Guardamos el estado actual
    ...
    const updateUser = async () => {
      try {
        ...
        const payload = {
          ...
          apellidos: apellidos.value,
          usuario: usuario.value,
          estado: estado.value // Incluimos el estado actual
        };
    ...
    const toggleEstado = async (idusuario, estado) => { // Cambié id a idusuario
      try {
        const token = store.state.auth.token;
        if (!token) throw new Error('No se encontró token de autenticación');
        await axios.patch(
    ...
    const deleteUser = async (idusuario) => { // Cambié id a idusuario
    ...
    const clearForm = () => {
      idcliente.value = '';
      perfil_id.value = '';
      nombre.value = ''; // Cambié a nombre
      apellidos.value = '';
      usuario.value = '';
      password.value = '';
      estado.value = true; // Restablecemos a true por defecto
      errorMessage.value = '';
    };
    ...
    return {
      ...
      usuario,
      password,
      estado, // Añadimos estado al return
      errorMessage,
      isEditing,
      createUser,
      editUser,
