<template>
    <div class="consulta-inventario">
      ...
      <!-- Tabla de Inventario -->
      <section v-if="mostrarInventario" class="form-section">
        <h2>Inventario de Productos</h2>
        <div class="form-actions">
          <button v-if="productosFiltrados.length" @click="exportarAExcel">
  ...
  export default {
    ...
    setup() {
      ...
      const hasPermission = (seccion, subseccion, permiso) => {
        return store.getters.hasPermission(seccion, subseccion, permiso);
      };
    ...
    computed: {
      // Computed property to handle the filtered products based on the current page
      resumenCostos() {
        const resumen = {};
    ...
    methods: {
      ...
      async consultar() {
        if (!this.hasPermission('inventario', 'consulta_inventario', 'ver')) {
      ...
      async consultarProductoEspecifico() {
        try {
          console.log('Iniciando consultarProductoEspecifico');
          console.log('Inputs:', {
            filtroProducto: this.filtroProducto,
            codigoDigitado: this.codigoDigitado,
            nombreDigitado: this.nombreDigitado,
          });
          ...
          console.log('Bodegas obtenidas:', todasLasBodegas);

          this.bodegas = todasLasBodegas;
          this.bodegasMostradas = todasLasBodegas;

          if (data.producto) {
            // Caso: consulta por código
            const inventario = data.inventario || [];
            console.log('Inventario para producto:', inventario);
            this.productos = [
              {
                codigo: data.producto.codigo,
                nombre: data.producto.nombre,
                cantidad_total: inventario.reduce((total, item) => total + (Number(item.cantidad) || 0), 0),
            ...
            // Caso: consulta por nombre
            console.log('Productos encontrados por nombre:', data.productos);
            ...
            console.warn('Respuesta inesperada, sin producto ni productos');
      ...
      async consultarTodosLosProductos() {
        try {
          console.log('Iniciando consultarTodosLosProductos');

          // Cargar todos los productos
          const fullResponse = await apiClient.get('/api/inventario-con-costos?limit=999999');
          console.log('Respuesta completa:', fullResponse.data);
          ...
          console.log('Todos los productos:', this.todosLosProductos);
          ...
          console.log('Productos paginados:', this.productos);
          this.productosFiltrados = [...this.productos];
          console.log('Productos filtrados iniciales:', this.productosFiltrados);
      ...
      filtrarPorBodega() {
        ...
        console.log('Productos filtrados después de filtrarPorBodega:', this.productosFiltrados);
      },
      filtrarPorEstado() {
        ...
        console.log('Productos filtrados después de filtrarPorEstado:', this.productosFiltrados);
      },
      ...
      cambiarPagina(nuevaPagina) {
        ...
        this.productos = this.todosLosProductos.slice(offset, offset + this.limite);
        this.productosFiltrados = [...this.productos];
        console.log('Productos filtrados después de cambiar página:', this.productosFiltrados);
      },
      sincronizarPorNombre() {
        ...
        const productoEncontrado = this.productosDisponibles.find((p) =>
          p.nombre.toLowerCase() === this.nombreDigitado.toLowerCase()
        );
      ...
      sincronizarCodigoConSelector() {
        ...
        const productoEncontrado = this.productosDisponibles.find((p) => p.codigo.toLowerCase() === this.codigoDigitado.toLowerCase());
        if (productoEncontrado) {
          this.filtroProducto = productoEncontrado.codigo;
      ...
      calcularCostoTotalInventario() {
        return Object.values(this.resumenCostos).reduce((sum, costo) => sum + costo, 0);
      },
      volverAlMenu() {
