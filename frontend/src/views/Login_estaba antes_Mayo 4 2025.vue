...
export default {
  ...
  setup() {
    ...
    const isPasswordVisible = ref(false); // Nueva variable para controlar visibilidad
    ...
    const login = async () => {
      ...
      try {
        ...
        const { token, id, idcliente, perfilid, permisos, clienteEstado } = response.data;
        ...
        if (idcliente) {
          if (clienteEstado === false) {
            ...
            loading.value = false;
            localStorage.removeItem('usuario_id'); // Limpiar si falla
            return;
          }
          store.commit('setAuth', { token, idcliente, perfilid, permisos });
          //console.log('Permisos seteados en store:', store.state.auth.permisos);
        ...
        localStorage.removeItem('usuario_id'); // Limpiar en caso de error
      } finally {
        loading.value = false;
      }
    };
...
input {
  width: 100%;
  padding: 12px 40px 12px 36px; /* Ajustado para dejar espacio al ícono de la derecha */
