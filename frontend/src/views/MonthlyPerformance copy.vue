    ...
    setup() {
      ...
      const chartOptions = ref({
        ...
        plugins: {
            ...
            callbacks: {
                label: (context) => {
                let label = context.label || ''; // Usamos context.label para el nombre del PDV
                if (label) {
                    label += ': ';
                }
                // Para gráficos de torta, usamos context.parsed directamente
                const value = context.parsed || 0; // Aseguramos que haya un valor, default a 0 si es undefined
