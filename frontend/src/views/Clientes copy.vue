...
export default {
  ...
  setup() {
    ...
    const correo = ref('');
    const logo = ref('');
    const estado = ref(true); // Estado por defecto: activo
      ...
      try {
        ...
        await axios.post(
          '/clientes',
          {
            ...
            estado: estado.value, // Enviar el estado
          },
          { headers: { Authorization: `Bearer ${token}` } }
        );
        clearForm();
    ...
    const editCliente = (cliente) => {
      ...
      estado.value = cliente.estado; // Cargar el estado del cliente
    };
      ...
      try {
        ...
        await axios.put(
          `/clientes/${editingClienteId.value}`,
          {
            ...
            correo: correo.value,
            logo: logo.value || null,
            estado: estado.value, // Enviar el estado actualizado
          },
          { headers: { Authorization: `Bearer ${token}` } }
        );
        clearForm();
    ...
    const clearForm = () => {
      ...
      correo.value = '';
      logo.value = '';
      estado.value = true; // Restaurar estado a "activo" por defecto
      errorMessage.value = '';
    };
