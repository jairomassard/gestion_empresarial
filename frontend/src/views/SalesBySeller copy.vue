<template>
    <div class="sales-by-seller">
      ...
      <div class="filters">
        ...
        <label>Asesor:</label>
        <select v-model="selectedSeller" @change="applySellerFilter">
    ...
    setup() {
      ...
      const applySellerFilter = () => {
        if (selectedSeller.value) {
          filteredSalesData.value = salesData.value.filter(row => row.seller === selectedSeller.value);
        } else {
      ...
      const horizontalBarChartData = computed(() => {
        ...
        const topSellers = [...salesData.value]
  ...
  .sales-table td:first-child { /* Columna "Asesor" */
    max-width: 200px;
    white-space: normal;
