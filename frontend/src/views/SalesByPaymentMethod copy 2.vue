...
export default {
  ...
  setup() {
    ...
    const day = ref(null); // Día seleccionado, null significa "todo el mes"
    const days = ref([]); // Lista de días disponibles para el mes/año seleccionado
    const status = ref('Activo');
    const selectedPdv = ref(null); // PDV seleccionado, null significa "todos los PDVs"
    const allPdvs = ref([]); // Lista de todos los PDVs con su estado
    const salesData = ref([]);
    const totals = ref({});
    ...
    const updateDays = () => {
      ...
      // Si el día seleccionado ya no es válido para el nuevo mes, restablecerlo
    ...
    const updatePdvsAndFetchData = () => {
      // Restablecer el PDV seleccionado si no está en la nueva lista filtrada
      ...
      try {
        const response = await axios.get('/dashboard/sales-by-payment-method', {
          params: {
            ...
            day: day.value,
            status: status.value,
            pdv: selectedPdv.value // Enviar el PDV seleccionado como parámetro
          }
        });
    ...
    const stackedBarChartData = computed(() => {
      const data = {
        labels: salesData.value.map(row => row.pdv), // Usar los PDVs de los datos filtrados
        datasets: [
          {
            label: 'Bonos',
    ...
    // Datos para el gráfico de pastel (distribución por medio de pago)
...
.charts-container {
  display: flex;
  flex-wrap: wrap;
  gap: 20px; /* Reducimos el espacio entre las gráficas */
  justify-content: center; /* Centramos las gráficas */
  max-width: 1200px; /* Limitamos el ancho máximo del contenedor */
...
.chart-wrapper {
  flex: 1;
  min-width: 300px;
  max-width: 600px; /* Reducimos el ancho máximo para que las gráficas estén más juntas */
  height: 400px;
}
