<template>
    ...
    setup() {
      ...
      const noDataMessage = computed(() => {
        if (selectedDay.value !== 'Todos') {
          return `No hay datos disponibles para el ${selectedDay.value} de ${monthName.value} ${year.value}.`;
        }
        return `No hay datos disponibles para ${monthName.value} ${year.value} con los filtros seleccionados.`;
      });
      ...
      const updateFilteredData = () => {
        console.log('Actualizando datos filtrados, selectedDay:', selectedDay.value);
        ...
        if (selectedDay.value === 'Todos') {
          ...
          console.log('Filtrando por día:', { datePattern, filteredData: filteredDailyProfitData.value });
        }
      ...
      watch(selectedDay, (newDay) => {
        console.log('Día seleccionado cambiado a:', newDay);
      });
  ...
  .table-container {
    max-width: 100%;
    overflow-x: auto;
    margin-top: 150px; /* Aumentado desde 40px para más separación */
  }
  ...
  .chart-wrapper {
    margin-bottom: 120px; /* Aumentado desde 60px para más separación entre gráficos */
    height: 400px;
  }
