...
export default {
  ...
  setup() {
    ...
    const exportToPDF = async () => {
      ...
      const tableWidth = 400; // Ajustar el ancho para que quepa dentro de los márgenes
      ...
      for (let i = 0; i < chartRows.length; i++) {
        ...
        for (let j = 0; j < chartWrappers.length; j++) {
          ...
          const chartCanvas = await html2canvas(chartWrappers[j], { scale: 2.5 });
          const chartImgData = chartCanvas.toDataURL('image/png');
          chartImages.push({ data: chartImgData, props: pdf.getImageProperties(chartImgData) });
        }

        // Añadir los gráficos al PDF
        const chartWidth = 95; // Ajustar el ancho para optimizar el espacio
        const chartHeight = (chartImages[0].props.height * chartWidth) / chartImages[0].props.width * 1.25; // Aumentar el alto para incluir los números del eje X
        const xPositions = [margin, margin + chartWidth + 7]; // Reducir el espacio entre gráficos

        for (let j = 0; j < chartImages.length; j++) {
          pdf.addImage(chartImages[j].data, 'PNG', xPositions[j], yPosition, chartWidth, chartHeight);
        }

        yPosition += chartHeight + 13;
...
.chart-wrapper {
  flex: 1;
  margin: 0 10px;
  height: 270px; /* Aumentar para asegurar que los gráficos se rendericen completamente */
  max-width: 600px;
}
