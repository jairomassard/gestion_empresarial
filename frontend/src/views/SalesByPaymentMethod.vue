...
// Registrar los componentes de Chart.js (sin el plugin de datalabels)
ChartJS.register(
  Title,
  Tooltip,
  Legend,
...
export default {
  ...
  setup() {
    ...
    // Variable para activar/desactivar el formato simplificado (ej. 33,5M)
    const useSimplifiedFormat = ref(false); // Cambia a false para mostrar el formato normal
    ...
    // Formatear moneda para la tabla (siempre en formato completo)
    const formatCurrency = (value) => {
      return new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0 }).format(value);
    };

    // Formatear valores para los gráficos (simplificado o normal según useSimplifiedFormat)
    const formatForCharts = (value) => {
      if (useSimplifiedFormat.value && value >= 1_000_000) {
        const millions = value / 1_000_000;
        return `${millions.toFixed(1).replace(/\.0$/, '')}M`; // Ej. 33,5M
      }
        ...
        y: {
          ...
          ticks: {
            callback: function(value) {
              return formatForCharts(value); // Usa formato simplificado o normal
            }
          },
          stacked: true
        }
        ...
        tooltip: {
          callbacks: {
            label: function(context) {
              ...
              label += formatForCharts(context.parsed.y); // Usa formato simplificado o normal
              return label;
            }
          }
        },
        datalabels: {
          display: false,  // en false no se muestra
          anchor: 'center', // Posicionar la etiqueta en el centro de la barra
          align: 'center', // Alinear el texto al centro
          color: '#000', // Color del texto (negro para que contraste con el fondo de la barra.  Clanco es #fff)
          font: {
            size: 9,
            weight: 'bold',
          },
        ...
        tooltip: {
          callbacks: {
            label: function(context) {
              ...
              label += formatForCharts(context.parsed); // Usa formato simplificado o normal
              return label;
            }
          }
        },
...
.charts-container {
  ...
  gap: 20px; /* Reducido para que el gráfico de barras tenga más espacio */
  justify-content: center;
  /* Aumenta este valor para permitir que el gráfico de barras se expanda más */
  max-width: 1600px; /* Aumentado para que el gráfico de barras pueda usar más espacio */
  margin: 0 auto;
}
...
.chart-wrapper.bar-chart {
  flex: 3; /* Aumentado para darle más prioridad al gráfico de barras */
  /* Ajusta el ancho del gráfico de barras aquí */
  min-width: 600px; /* Aumentado para asegurar un ancho mínimo mayor */
  max-width: 1200px; /* Ancho máximo - debería reflejarse ahora */
}
